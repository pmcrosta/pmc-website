...
Official enrollment trend data is also stored in a [CSV](datasets/enr_all.csv).
...
comb <- ccdnjk12_peers %>%
  ...
  dplyr::left_join(enr_all %>% dplyr::select(ST_LEAID, OfficialTotalEnrollment=TotalEnrollment), by="ST_LEAID")
...
write_csv(comb, file="datasets/comb.csv")
