...
rmarkdown::paged_table(ccdnjk12 %>% dplyr::filter(hc_euclid_ward) %>% dplyr::select(LEA_NAME),
  options = list(
    rows.print = 10
  )
)
...
rmarkdown::paged_table(ccdnjk12 %>% dplyr::filter(hc_pc_dist_unscaled_ward) %>% dplyr::select(LEA_NAME),
  options = list(
    rows.print = 10
  )
)
