...
Finally, I add the possible peer sets back to our CCD data set and save them as [RDS](datasets/ccdnjk12_peers.rds) or [CSV](datasets/ccdnjk12_peers.csv) files for easy access.
...
# Serialize
saveRDS(ccdnjk12_peers, file="datasets/ccdnjk12_peers.rds")
...
The table below summarizes a selection of district characteristics for the districts that are included in each potential peer set. Fair Haven itself is included in the last row for comparison. The `Peninsula` column is TRUE if FH, Rumson, Little Silver, and Shrewsbury are all included in that particular peer set.
...
peerset_summary <- bind_rows(
  ccdnjk12_peers %>% 
    ...
    dplyr::summarise(Method="Euclidean Distance",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(mah_score <= 100) %>%
    dplyr::summarise(Method="Mahalanobis Distance",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
  ...
  ccdnjk12_peers %>% 
    ...
    dplyr::summarise(Method="Euclidean Ward",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(hc_euclid_mcquitty) %>%
    dplyr::summarise(Method="Euclidean WPGMA",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(hc_pc_dist_scaled_ward) %>%
    dplyr::summarise(Method="PC Scaled Ward",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(hc_pc_dist_scaled_mcquitty) %>%
    dplyr::summarise(Method="PC Scaled WPGMA",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(hc_pc_dist_unscaled_ward) %>%
    dplyr::summarise(Method="PC Unscaled Ward",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
    dplyr::filter(hc_pc_dist_unscaled_mcquitty) %>%
    dplyr::summarise(Method="PC Unscaled WPGMA",
                     Districts=n(),
                     Peninsula = all(c(fh_leaid, ls_leaid, rums_leaid, shrews_leaid) %in% LEAID),
                     across(c(Edu_Bach_or_Higher, Enroll_Elem, Fam_Avg_Size, HH_Married_Children, Pop_Under_18, HH_Med_Income, Occ_Mgmt_Bus_Sci_Arts), \(x) round(mean(x), 1))),
  ccdnjk12_peers %>% 
...
peerset_summary %>% 
  ...
  tab_source_note("Peer group stats for Fair Haven for each method. Data from ACS-EDGE.")
