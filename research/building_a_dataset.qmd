...
Student outcomes data come from the [NJ School Performance Reports](https://rc.doe.state.nj.us/download). To gather longitudinal data, I ingest report card data from 2014-15 to 2023-24. The content and format of the report cards varies greatly from year to year. I makea best effort approach to join appropriately across years.

```{r}
#| label: report-cards-2324
#| echo: true
#| message: false
#| warning: false
#| eval: true
#| cache: true
#| output: false
#| code-summary: Code - Read in NJ Report Card data 2023-24
...
Essential report card data are joined into one report card data set, saved as a [CSV](datasets/rc_all_comb.csv) for easy access.
...
#| dependson: report-cards-1415, report-cards-1516, report-cards-1617, report-cards-1718, report-cards-1819, report-cards-1920, report-cards-2021, report-cards-2122, report-cards-2223, report-cards-2324 
...
# Enrollments {#sec-enrollments}

Official enrollment trend data is retrieved from the NJ Department of Education and stored in a [CSV](datasets/enr_all.csv).
...
Though there are times when it will be useful for individual data sets to be used alone, it is also useful to have [one dataset](datasets/comb2324.csv) that contains the latest data available, which is the 2023-24 school year as of this writing.
...
#| cache: true
#| output: false
#| code-summary: Code - Bringing data together

comb2324 <- ccdnjk12_peers %>%
  dplyr::left_join(per_pupil_exp, by="ST_LEAID") %>%
  dplyr::left_join(rc_comb2324, by="ST_LEAID") %>%
  dplyr::left_join(enr_all %>% dplyr::filter(SchoolYear=="2023-2024") %>% dplyr::select(ST_LEAID, OfficialTotalEnrollment=TotalEnrollment) , by="ST_LEAID")
...
| Name | Description |
|----|----|
| [ccdnjk12_peers.csv](datasets/ccdnjk12_peers.csv) | Common Core of Data (2023-24), ACS-EDGE, and FH peer group sets. Only contains PK/K-8 districts |
| [comb2324.csv](datasets/comb2324.csv) | Common Core of Data, ACS-EDGE, Budget data, Report Card data, all for PK/K-8 districts 2023-24 |
| [edge_df.csv](datasets/edge_df.csv) | ACS-EDGE for all NJ districts |
| [enr_all.csv](datasets/enr_all.csv) | NJ District Enrollment counts from 2013-2014 to 2023-2024 school years, all NJ districts |
| [per_pupil_exp.csv](datasets/per_pupil_exp.csv) | Per pupil expenditures for 2023-24 from 2022-23 User Friendly Budgets, all NJ districts |
| [rc_all_comb.csv](datasets/rc_all_comb.csv) | Report Card data for all NJ districts from 2014-15 to 2023-24 |
